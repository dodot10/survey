...
<body class="bg-gray-100 p-6">
    <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        <h1 class="text-2xl font-bold mb-4">Edit Survei Tempat Ujian</h1>
        <form action="{{ route('surveys.update', $survey->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2">Nama Tempat</label>
                <input type="text" name="name" value="{{ $survey->name }}" class="w-full border px-4 py-2 rounded" required>
            </div>
            <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2">Lokasi</label>
                <textarea name="location" class="w-full border px-4 py-2 rounded" required>{{ $survey->location }}</textarea>
            </div>
